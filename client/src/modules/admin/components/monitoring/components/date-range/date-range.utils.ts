import { addDays, differenceInCalendarDays, endOfDay, isAfter, isBefore, startOfDay, startOfMonth, subMonths } from "date-fns";
...
export const clampRangeToBounds = (
  ...
  const from = clampDate(normalized.from, minDate, maxDate);
  const to = clampDate(normalized.to, minDate, maxDate);
