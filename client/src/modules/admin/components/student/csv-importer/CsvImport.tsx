...
import { CloudUpload, Download, FileText, X, AlertCircle, Upload } from "lucide-react";
...
export function CsvImport({
  ...
  previewRowCount = 3,
  showSimulateError: _showSimulateError = false,
  fullWidth = false,
  onParsed,
  onImport,
}: CsvImportProps) {
        ...
        <div className="flex items-center gap-4 p-5 border-b border-border">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {formatBytes(parsed.fileSize)} · {parsed.rows.length} filas de datos · UTF-8
            </p>
          </div>
          <button
            type="button"
      ...
      >
        ...
        >
          <span className="text-center">
            Formato aceptado: CSV · Máx. {formatBytes(maxSizeBytes)}
          </span>
          {!fullWidth ? (
            <>
              <span className="hidden sm:inline">·</span>
              <span className="flex items-center gap-1">
                <FileText className="h-3.5 w-3.5" /> Solo .CSV
              </span>
              <span>·</span>
