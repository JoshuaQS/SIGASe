...
// (Legacy) heights used by the old preview panel.
...
function PreviewPanel({ state }: { state: FilterState }) {
  ...
  const supportsRanking = supportsRankingForState(state)
  const isStudentIndividualScope = state.filterType === 'alumno' && state.scope === 'individual'
  const isStudentAllScope = state.filterType === 'alumno' && state.scope === 'todos'
  const isCareerIndividualScope = state.filterType === 'carrera' && state.scope === 'individual'
  const isCareerMultiScope = state.filterType === 'carrera' && state.scope === 'varias'
  const isCareerAllScope = state.filterType === 'carrera' && state.scope === 'todas'
  const isCareerScope = isCareerIndividualScope || isCareerMultiScope || isCareerAllScope
  const isCareerGroupScope = isCareerMultiScope || isCareerAllScope
  const hasStudentSelected = Boolean(state.selectedStudent)
  const hasCareerSelected = state.scope === 'individual' ? state.selectedCareers.length > 0 : true
  const hasAccessTypeSelected = Boolean(state.accessType)
  const hasDateRangeSelected = state.dateFilter && Boolean(state.dateRange.from && state.dateRange.to)
  ...
  const showTable =
    !isInitialStage &&
    !showRanking &&
    (state.filterType === 'carrera' || state.scope === 'todos' || state.scope === 'todas' || state.scope === 'varias')

  return (
    <div className="w-1/2 min-h-0">
      {/* The preview has many branches; keep it as-is from the design */}
      {/* Student individual */}
              ...
              <div className="mb-2 flex items-center gap-2">
                ...
                <span
                  className={cn(
                    ...
                    isFailedOnly ? 'bg-rose-200/70 text-rose-500' : 'bg-primary/20 text-muted-foreground',
                  )}
                >
                  Contexto análisis
                </span>
            ...
            {hasStudentSelected && hasAccessTypeSelected ? (
              <div className="grid grid-cols-4 gap-3">
                {individualKpiLabels.map((label, index) => (
                  <div key={label} className="space-y-2 rounded-lg border bg-card p-3">
                    <span
                      className={cn(
                    ...
                    <div
                      className={cn(
                        'h-6 w-12 rounded-md',
                        isFailedOnly || (isBothResults && index === 3) ? 'bg-rose-300/80' : 'bg-primary/35',
                      )}
                    />
                  </div>
                ))}
            ...
            {showRanking ? (
              <div className="rounded-lg border bg-card p-4">
                <div className="mb-3 flex items-center gap-2">
                  <TrendingUp className={cn('h-4 w-4', isFailedOnly ? 'text-rose-500' : 'text-primary')} />
                  <span className={cn('text-xs font-medium', isFailedOnly && 'text-rose-500')}>
                    Top {state.topN}
                  </span>
                </div>
                {Array.from({ length: Math.min(state.topN, 8) }).map((_, i) => (
              ...
              <div className="rounded-lg border bg-card p-4">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <BarChart3 className={cn('h-4 w-4', isFailedOnly ? 'text-rose-500' : 'text-primary')} />
                    <span
                      className={cn(
                        'inline-flex h-5 items-center rounded-full px-2 text-[10px] font-semibold leading-none',
                        isFailedOnly ? 'bg-rose-200/70 text-rose-500' : 'bg-primary/20 text-muted-foreground',
                  ...
                  <div className="flex items-center gap-2">
                    {hasDateRangeSelected ? (
                      <div className="flex items-center gap-2">
                        <CalendarDays className={cn('h-4 w-4', isFailedOnly ? 'text-rose-500' : 'text-muted-foreground')} />
                        <span
                          className={cn(
                            'inline-flex h-5 items-center rounded-full px-2 text-[10px] font-semibold leading-none',
                            isFailedOnly ? 'bg-rose-200/70 text-rose-500' : 'bg-primary/20 text-muted-foreground',
                    ...
                    {state.sortOrder === 'asc' ? (
                      <ArrowUpDown className={cn('h-4 w-4', isFailedOnly ? 'text-rose-500' : 'text-muted-foreground')} />
                    ) : (
                      <ArrowDownUp className={cn('h-4 w-4', isFailedOnly ? 'text-rose-500' : 'text-muted-foreground')} />
                    )}
                  </div>
                </div>
            ...
            {hasCareerSelected ? (
              <div className="space-y-4">
                <div className="grid grid-cols-4 gap-3">
                  {(isBothResults
                    ? [
                        isCareerAllScope ? 'Todas las carreras' : isCareerMultiScope ? 'Carreras' : 'Carrera',
                        'Totales',
                        'Exitosos',
                        'Fallidos',
                      ]
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-4 gap-3">
              {Array.from({ length: 4 }).map((_, id) => (
                <div key={id} className="space-y-2 rounded-lg border bg-card p-3">
                  {isInitialStage ? (
                    <>
                    ...
                    <>
                      ...
                      <Skeleton variant="value" size="value" tone={stage === 'advanced' ? 'primary' : 'muted'} />
                    </>
                  )}
                </div>
              ))}
            ...
            <div className="rounded-lg border bg-card p-4">
              <div className="mb-3 flex items-center gap-2">
                <BarChart3 className={cn('h-4 w-4', isInitialStage || stage === 'advanced' ? 'text-primary' : 'text-muted-foreground')} />
              ...
              <div className="flex h-32 items-end gap-1">
                {PREVIEW_BAR_HEIGHTS_PCT.map((pct, i) => (
                  <div
                    key={i}
                    className={cn('flex-1 rounded-t transition-all', isInitialStage || stage === 'advanced' ? 'bg-primary/40' : 'bg-muted')}
                    style={{ height: `${pct}%` }}
                  />
                ))}
              </div>
            ...
            {showSplit ? (
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-2 rounded-lg border bg-card p-3">
                  <div className={cn('h-3 w-20 rounded-full', state.accessType === 'ambos' ? 'bg-rose-200/70' : 'bg-primary/25')} />
                  <div className={cn('h-16 w-full rounded-md', state.accessType === 'ambos' ? 'bg-rose-300/80' : 'bg-primary/40')} />
                </div>
              </div>
            ) : null}
              ...
              <div className="space-y-3 rounded-lg border bg-card p-4">
                ...
                {Array.from({ length: Math.min(state.topN, 5) }).map((_, i) => (
                  <div key={i} className="flex items-center gap-2 py-1.5">
                    ...
                    <Skeleton variant="pill" size="line" tone="primary" className="flex-1 opacity-70" />
                  </div>
                ))}
              </div>
            ) : null}
            ...
            {showTable ? (
              <div className="space-y-3 rounded-lg border bg-card p-4">
                <div className="flex items-center gap-2">
                  <Table className="h-4 w-4 text-muted-foreground" />
                  <Skeleton variant="pill" size="pillMd" tone={isInitialStage ? 'success' : 'muted'} />
                </div>
                {Array.from({ length: 4 }).map((_, row) => (
                  <div key={row} className="grid grid-cols-[1fr_4fr_1fr] gap-2">
...
export default function DashboardAnalysisComposer({
  open,
  onOpenChange,
  metadata = null,
  resetSignal = 0,
  ...
  const [careerCatalogCache, setCareerCatalogCache] = useState<DashboardCareerSearchItem[] | null>(null)
  ...
  const [selectedStudentSnapshot, setSelectedStudentSnapshot] = useState<DashboardStudentSearchItem | null>(null)
  ...
  const selectedStudentItem = useMemo(
    () => students.find((item) => item.id === composer.state.selectedStudent) ?? selectedStudentSnapshot ?? null,
  ...
  const selectedCareerLabel = useMemo(() => {
    ...
    return careers.find((career) => career.id === selectedId)?.displayLabel ?? selectedId
  }, [careers, composer.state.selectedCareers])

  const visibleCareerBadgeIds = useMemo(() => {
    const maxShownItems = 2
    return careerBadgesExpanded ? composer.state.selectedCareers : composer.state.selectedCareers.slice(0, maxShownItems)
  }, [careerBadgesExpanded, composer.state.selectedCareers])
  const hiddenCareerBadgeCount = composer.state.selectedCareers.length - visibleCareerBadgeIds.length
  ...
  useEffect(() => {
    if (!composer.state.selectedStudent) setSelectedStudentSnapshot(null)
  ...
  }, [composer.state.filterType, composer.state.scope, deferredCareerSearch, careerComboboxOpen, careerCatalogCache])
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent animation="fade" size="4" className="max-h-[92vh] max-w-[1240px] overflow-hidden p-0">
        <DialogHeader className="border-b px-6 pb-4 pt-6">
          <DialogTitle className="text-lg">Compositor de análisis</DialogTitle>
          <DialogDescription>
            Configura filtros y criterios antes de ejecutar el análisis.
          </DialogDescription>
        </DialogHeader>
                          ...
                          <div className="space-y-2">
                            ...
                            >
                              {composer.state.filterType === 'alumno' ? (
                                <>
                                  <Button
                                    variant={composer.state.scope === 'individual' ? 'primary' : 'outline'}
                                    size="sm"
                                    className="w-full"
                                    onClick={() => composer.setScope('individual')}
                                  >
                                    Individual
                                  </Button>
                                  <Button
                                    variant={composer.state.scope === 'todos' ? 'primary' : 'outline'}
                                    size="sm"
                                    className="w-full"
                                ...
                                <>
                                  ...
                                  <Button
                                    variant={composer.state.scope === 'varias' ? 'primary' : 'outline'}
                                    size="sm"
                                    className="w-full"
                                  ...
                                  <Button
                                    variant={composer.state.scope === 'todas' ? 'primary' : 'outline'}
                                    size="sm"
                                    className="w-full"
                          ...
                          <div className="space-y-3 border-t pt-3">
                            {composer.state.filterType === 'alumno' && composer.state.scope === 'individual' ? (
                              <div className="space-y-2">
                                ...
                                {composer.state.selectedStudent && selectedStudentItem && studentSearch.trim().length < 2 ? (
                                  <div className="h-40 rounded-md border bg-muted/20 px-3 py-2">
                                    <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                                      Alumno seleccionado
                                    </p>
                                    <p className="mt-2 text-sm font-medium">{selectedStudentItem.displayLabel}</p>
                                    <p className="mt-1 text-xs text-muted-foreground">
                                      Matrícula: {selectedStudentItem.enrollmentId}
                                    </p>
                                  </div>
                                ) : (
                                    ...
                                    {studentsLoading ? (
                                      ...
                                      students.map((student) => (
                                        <button
                                          ...
                                          onClick={() => {
                                            composer.setSelectedStudent(student.id)
                              ...
                              <div className="space-y-2">
                                <Popover
                                  ...
                                  onOpenChange={(openValue) => {
                                    setCareerComboboxOpen(openValue)
                                    if (openValue) setCareersLoading(true)
                                  }}
                                >
                                    ...
                                    >
                                      <div className="flex flex-wrap items-center gap-1 pr-2.5 text-left">
                                        {composer.state.scope === 'individual' ? (
                                          selectedCareerLabel ? (
                                            <Badge variant="outlined" className="rounded-sm text-xs">
                                              {selectedCareerLabel}
                                            </Badge>
                                          ) : (
                                            <span className="text-sm text-muted-foreground">Selecciona carrera</span>
                                          )
                                        ) : composer.state.selectedCareers.length > 0 ? (
                                          <>
                                            {visibleCareerBadgeIds.map((careerId) => {
                                              const label = careers.find((career) => career.id === careerId)?.displayLabel ?? careerId
                                              return (
                                                <Badge key={careerId} variant="outlined" className="rounded-sm text-xs">
                                                  {label}
                                                  <span
                                                    role="button"
                                                    tabIndex={0}
                                                    ...
                                                    onClick={(event) => {
                                                      ...
                                                      composer.toggleCareer(careerId)
                                                    }}
                                                    onKeyDown={(event) => {
                                            ...
                                            {hiddenCareerBadgeCount > 0 || careerBadgesExpanded ? (
                                              <Badge
                                                variant="outlined"
                                                className="cursor-pointer rounded-sm text-xs"
                                                onClick={(event) => {
                                                  event.stopPropagation()
                                                  setCareerBadgesExpanded((prev) => !prev)
                                                }}
                                              >
                                                {careerBadgesExpanded ? 'Ver menos' : `+${hiddenCareerBadgeCount} más`}
                                              </Badge>
                                            ) : null}
                                          </>
                                        ) : (
                                          <span className="text-sm text-muted-foreground">Selecciona carreras</span>
                                        )}
                                      </div>
                                  ...
                                  >
                                    <Command className="overflow-hidden">
                                      ...
                                      <CommandList className="max-h-56 overflow-y-auto overscroll-contain">
                                        {careersLoading ? (
                                          <CommandGroup>
                                            <div className="inline-flex items-center gap-2 px-2 py-2 text-xs text-muted-foreground">
                                              <Loader2 className="h-3.5 w-3.5 animate-spin" />
                                              Buscando carreras...
                                            </div>
                                          </CommandGroup>
                                          ...
                                          <CommandGroup>
                                            {careers.map((career) => {
                                              ...
                                              return (
                                                ...
                                                >
                                                  <span className="truncate">{career.displayLabel}</span>
                                                  {selected ? <CheckIcon size={16} className="ml-auto" /> : null}
                                                </CommandItem>
                                              )
                                            })}
                                          </CommandGroup>
                            ...
                            <Select
                              onValueChange={(value) => composer.setAccessType(value as AccessType)}
                              ...
                              <div className="space-y-2">
                                {composer.state.accessType ? (
                                  <Button
                                    ...
                                    onClick={() =>
                                      composer.setSortOrder(composer.state.sortOrder === 'desc' ? 'asc' : 'desc')
                                    }
                                  >
                                    ...
                                    {composer.state.sortOrder === 'desc' ? 'Mayor a menor' : 'Menor a mayor'}
                                  </Button>
                                ) : null}
                              ...
                              <div className="space-y-2">
                                ...
                                {composer.state.dateFilter ? (
                                  <DateRangeSelector
                                    ...
                                    onChange={(range) =>
                                      composer.setDateRange({ from: range?.from, to: range?.to })
                                    }
                                  />
                                ) : null}
                              </div>
