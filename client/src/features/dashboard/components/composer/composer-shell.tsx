...
import { ChevronRight, Download, FileSpreadsheet, FileText, RotateCcw, Sparkles } from 'lucide-react'
...
const ComposerShell = ({
  ...
  const spawnRipple = useCallback((event: PointerEvent<HTMLDivElement>) => {
  ...
  const summaryLabel = 'Ajusta el filtrado para poder visualizar y descargar las estadísticas de accesos a eLibro mostradas dinámicamente en el panel de monitoreo.'
  ...
  const canShowInlineActions = hasAppliedFilter && Boolean(currentAnalysis) && Boolean(onExport)
    ...
    <Card className='border-0 bg-transparent p-0 shadow-none'>
      <div
        ...
        className={cn(
          ...
          // Keep the shadow on the wrapper so it doesn't get clipped.
          'h-[92px] bg-card shadow-lg ring-1 ring-border/50 transition-[box-shadow] duration-300 ease-in-out',
          !disabled && metadata && 'hover:shadow-md',
        )}
      >
        <div className="relative z-10 flex h-full w-full items-center px-4 py-3">
        <span className='pointer-events-none absolute inset-0 z-[5] overflow-hidden rounded-[inherit]' aria-hidden>
          {ripples.map((ripple) => (
            <span
              key={ripple.id}
        ...
        <div className='flex min-w-0 flex-1 items-center gap-3 sm:gap-4'>
          ...
          <div className='min-w-0 flex-1 flex flex-col gap-1'>
            <CardTitle className='flex flex-wrap items-center gap-1 text-xl font-semibold sm:text-2xl'>
              ...
              >
                <defs>
                  <linearGradient id={arrowGradId} x1="0" y1="0" x2="24" y2="24" gradientUnits="userSpaceOnUse">
                    <stop offset="0%" stopColor="#ffaa40" />
