...
interface MonitoringComposerBarProps {
  ...
  onQuickApply?: (value: ComposerDraftState) => void;
}
...
export function MonitoringComposerBar({ value, onChange, minDate, topNOptions = TOP_N_OPTIONS, onQuickApply }: MonitoringComposerBarProps) {
          ...
          <>
            ...
            {value.type === "careers" ? (
              <CareerMultiComboboxField
                ...
                onChange={(careers) => {
                  const isMulti = careers.length >= 2;
                  const isSingle = careers.length === 1;
                  const nextMode = isMulti ? "all-careers" : "single-career";

                  if (isMulti) {
                    // Entering career_multi: ensure sortDirection is set and topN is valid
                    const currentTopN = value.topN;
                    const normalizedTopN =
                      value.topEnabled && currentTopN !== undefined && ![5, 10].includes(currentTopN)
                        ? 5
                        : currentTopN;
                    updateState({
                      careers,
                      ...
                      ...(normalizedTopN !== currentTopN ? { topN: normalizedTopN } : {}),
                    });
                  } else {
                    // Entering career_single or no selection: clear ranking fields
                    updateState({
                      careers,
                      mode: isSingle ? "single-career" : "all-careers",
                      sortDirection: undefined,
                      topEnabled: false,
                      topN: undefined,
                    });
