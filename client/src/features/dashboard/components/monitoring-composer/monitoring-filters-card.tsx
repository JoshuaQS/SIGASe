...
import { validateComposerDraft } from "./composer.schema";
...
export function MonitoringFiltersCard({
  ...
  const canFilter = validateComposerDraft(composerValue).valid;
  const handleFilter = (target?: ComposerDraftState) => {
    const value = target ?? composerValue;
    if (!validateComposerDraft(value).valid || isFiltering) return;
    setIsFiltering(true);
    onFilteringChange?.(true);
    setComposerValue({ ...value, didFilter: false });
    window.setTimeout(() => {
      const next = { ...value, didFilter: true };
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-stretch">
          ...
          <div className="min-w-0 flex-1 rounded-xl border border-border/70 bg-background p-3">
            <MonitoringComposerBar
              ...
              onQuickApply={(quickState) => handleFilter(quickState)}
            />
          </div>
        </div>
