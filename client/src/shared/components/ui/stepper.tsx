...
const H_STEP_CONNECTOR = 'h-0.5 w-5 shrink-0 transition-colors duration-300 sm:w-7'
const H_STEP_SPACER = 'w-5 shrink-0 sm:w-7'
const H_STEP_COL = 'flex w-[5.25rem] shrink-0 flex-col items-center sm:w-28'

function stepStatus(i: number, currentStep: number): 'completed' | 'active' | 'upcoming' {
            ...
            <div className={cn(H_STEP_COL, 'min-w-0 px-0.5 text-center')}>
              <p
                className={cn(
                  ...
                  status === 'upcoming' ? 'font-medium text-muted-foreground' : 'text-foreground',
                )}
              >
                {step.label}
              </p>
              {step.description ? (
                <p
                  className={cn(
                    ...
                    status === 'upcoming' ? 'text-muted-foreground/75' : 'text-muted-foreground',
                  )}
                >
                  {step.description}
                </p>
  ...
  const indicator = (
    <div
      className={cn(
        ...
        base,
        status === 'completed' && 'border-primary bg-primary text-primary-foreground shadow-sm',
        status === 'active' &&
...
export function Stepper({
                ...
                <div className='min-w-0 pb-7 pt-0.5'>
                  <p
                    className={cn(
                      ...
                      status === 'upcoming' && 'font-medium text-muted-foreground',
                    )}
                  >
                    {step.label}
                    {step.optional && (
  ...
  return (
    <div className={cn('w-full', className)}>
      ...
      {showLabels ? (
        <StepperHorizontalLabels steps={steps} currentStep={currentStep} className='mt-4' />
      ) : null}
    </div>
  )
}
