...
export interface GlareHoverProps extends ComponentProps<"div"> {
  ...
  playOnce?: boolean
  /**
   * When `true`, the glare keeps sweeping while hovered.
   * Uses the `glare-sweep` keyframes from `global.css`.
   */
  loopOnHover?: boolean
}
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // LOOP (overrides transition behavior)
        loopOnHover && "before:transition-none hover:before:animate-[glare-sweep_var(--gh-duration)_linear_infinite]",
        // TRANSITION
        !loopOnHover &&
        !playOnce &&
          "before:transition-[background-position] before:duration-[var(--gh-duration)] before:ease-in-out",
        // PLAY ONCE: run a single sweep (diagonal) per hover.
        !loopOnHover &&
        playOnce &&
          "before:transition-none hover:before:animate-[glare-sweep-diagonal_var(--gh-duration)_ease-in-out_1]",
        // HOVER EFFECT
        !loopOnHover && !playOnce && "hover:before:[background-position:100%_100%,0_0]",
        className
      )}
      style={cssVars}
    >
